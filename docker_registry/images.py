...
def require_completion(f):
    ...
    def wrapper(*args, **kwargs):
        if store.exists(store.image_mark_path(kwargs['image_id'])):
            return toolkit.api_error('Image is being uploaded, retry later',409)
...
def put_image_layer(image_id):
    try:
        ...
        logger.debug('put_image_layer: json_data:%s' %json_data)
    except exceptions.FileNotFoundError:
        return toolkit.api_error('Image not found', 404)
...
def save_checksums(image_id, checksums):
    ...
    checksumjson = json.dumps(checksums)
    logger.debug("[save_checksums]:checksumjson={0}".format(checksumjson))
    store.put_content(checksum_path, checksumjson)
...
def put_image_json(image_id):
    ...
    try:
        ...
        pass
    logger.debug('put_image_json: json_data:%s' %flask.request.data)
